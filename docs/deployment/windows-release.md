...
空格与 Unicode 字符；`.sha256` 使用无 BOM UTF-8，文件名含 Unicode 时仍可由标准校验工具读取。
...
`.sha256`，保留 14 天。上传前工作流还会严格比对校验文件、解压实际 ZIP，并再次运行解压包内
验收器，确保验证对象与上传对象一致。CI 没有游戏数据，因此不能替代下一节的真实 data root 验收。
