...
和基础 `.blend` 都保持不变。Blender 导入器保留 glTF Action 和 Action Slot，不创建额外
NLA track，也不把多个动作串成一条时间轴。Blender 5.2 可直接切换多槽 Action，并同步
驱动骨架与辅助对象；场景起止帧只取所有导入 Action 原始范围的并集。导出的文件内嵌
`endfield_action_switcher.py` 注册文本；允许可信脚本执行后，3D 视图右侧 `Endfield`
面板可选择动作并一次性为骨架、衣物、辅助对象和 Shape Key 分配各自的 Action Slot。
切换器不会创建 NLA 数据。所有导入 Action 均设置 Fake User，以免当前未绑定的动作在
保存文件时被 Blender 当作无引用数据清理。
