...
- 随包验收器的不可变边界从“启动前校验”收紧为 worker/服务运行前后双重校验：第二轮再次检查
  ...
  提交 `aea2013` 又从 Unicode clean checkout 正式发布，包内 Windows PowerShell 5.1 验收器
  连接真实 data root 后生成含 `postRunIntegrity: verified` 的成功报告。
- Windows PowerShell 5.1 的 Unicode 归档实测继续发现 `.sha256` 原以 ASCII 写入，哈希正确但
  ...
  完整且记录哈希与 55,382,908 字节归档一致。Windows PowerShell 5.1 解压后，包内验收器通过
  中文 data root 入口复核 353 个文件，服务、索引、Manifest 和 11 项 worker 能力全部通过。
- Unicode 路径发布审计发现 ACL 构建临时批处理原以 ASCII 写入，会把 checkout 中的中文替换为
  ...
  中文的 clean checkout 完成依赖恢复、ACL、PyInstaller 与 .NET 发布；产物安装在中文路径后，
  包内 Windows PowerShell 5.1 验收器又通过中文 data root 入口写出中文报告，353 个文件、服务、
  索引、Manifest 和 11 项 worker 能力全部通过。
- 发布候选的 PE 依赖审计确认：冻结 Python 的 `VCRUNTIME140.dll`、`VCRUNTIME140_1.dll` 和
  ...
  Redistributable。产品运行边界明确收窄为 Windows 10/11 x64。提交 `c82dfdd` 已从独立 clean
  checkout 经过该门禁重新发布；包内 Windows PowerShell 5.1 验收器复核 353 个文件并连接真实
  data root，服务、索引、Manifest 和 11 项 worker 能力全部通过。
...
- 发布验收器可用 `-ReportPath` 生成稳定的 `schemaVersion: 1` 机器验收报告。报告只在真实
  ...
  正向报告验证通过；重复路径和发布目录内部路径分别被“拒绝覆盖”和“不可写入不可变包”
  门禁挡住。验收器现随产品放在 `tools/` 并进入不可变文件清单，新机器只需要发布包和游戏
  数据即可执行；对应发布契约已有自动化门禁。另一台机器执行后可直接保存报告作为 P5 证据。
