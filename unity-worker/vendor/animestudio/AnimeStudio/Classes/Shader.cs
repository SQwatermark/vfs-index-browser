...
namespace AnimeStudio
{
    ...
    public class SubShaderBinaryData : NamedObject
    {
        ...
        {
            m_CompressedBlob = ShaderBlobReader.ReadUInt32Array(reader, "m_CompressedBlob");
            m_Offsets = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_Offsets");
            m_CompressedLengths = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_CompressedLengths");
            m_DecompressedLengths = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_DecompressedLengths");
        }
    }
    ...
    public class SubShaderBlob
    {
        ...
        public SubShaderBlob(ObjectReader reader)
        {
            ...
            m_CompressedBlob = ShaderBlobReader.ReadByteArray(reader, "m_CompressedBlob");
            reader.AlignStream();
            m_Offsets = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_Offsets");
            m_CompressedLengths = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_CompressedLengths");
            m_DecompressedLengths = ShaderBlobReader.ReadUInt32ArrayArray(reader, "m_DecompressedLengths");
    ...
    {
        internal static byte[] ReadByteArray(ObjectReader reader, string fieldName)
        {
            var count = ReadCount(reader, 1, fieldName);
            return reader.ReadBytes(count);
        }

        internal static uint[] ReadUInt32Array(ObjectReader reader, string fieldName)
        {
            var count = ReadCount(reader, sizeof(uint), fieldName);
        ...
        internal static uint[][] ReadUInt32ArrayArray(ObjectReader reader, string fieldName)
        {
            var count = ReadCount(reader, sizeof(int), fieldName);
            var values = new uint[count][];
            for (var index = 0; index < count; index++)
            {
                values[index] = ReadUInt32Array(reader, $"{fieldName}[{index}]");
            }
            return values;
        }

        private static int ReadCount(ObjectReader reader, int minimumBytesPerItem, string fieldName)
        {
            var count = reader.ReadInt32();
            var maximum = reader.BytesLeft() / minimumBytesPerItem;
            if (count < 0 || count > maximum)
            {
                throw new InvalidDataException(
                    $"Invalid {fieldName} count {count}; maximum possible count is {maximum} " +
                    $"with {reader.BytesLeft()} bytes remaining.");
            }
            return count;
        }
    }
    ...
    public class Shader : NamedObject
    {
        ...
        public bool m_CompiledDataIncomplete;
        public string m_CompiledDataError;
        ...
        public Shader(ObjectReader reader) : base(reader)
        {
            if (version[0] == 5 && version[1] >= 5 || version[0] > 5) //5.5 and up
            {
                ...
                try
                {
                    ...
                    {
                        // 正式版将各 LOD 的编译载荷直接存放在 Shader 对象内。
                        m_EnableShaderLODStreaming = reader.ReadBoolean();
                        reader.AlignStream();

                        int numSubShaderBlobs = reader.ReadInt32();
                        if (numSubShaderBlobs < 0 || numSubShaderBlobs > reader.BytesLeft() / 20)
                            throw new InvalidDataException($"Invalid Endfield sub-shader blob count {numSubShaderBlobs}.");
                    ...
                    {
                        // CB3 使用独立 SubShaderBinaryData 对象，由 PPtr 关联到 Shader。
                ...
                catch (System.IO.EndOfStreamException) when (
                    reader.Game.Type.IsArknightsEndfieldCB3() ||
                    reader.Game.Type.IsArknightsEndfield())
                {
                    m_CompiledDataIncomplete = true;
                    m_CompiledDataError =
                        $"Unexpected end of Shader object with {reader.BytesLeft()} bytes remaining.";
                    Logger.Warning(
                        $"Shader compiled data is incomplete for {reader.assetsFile.fileName} " +
                        $"at path {reader.m_PathID}: {m_CompiledDataError}");
                }
                ...
                {
                    m_CompiledDataIncomplete = true;
                    m_CompiledDataError = ex.Message;
                    Logger.Warning(
                        $"Shader compiled data is invalid for {reader.assetsFile.fileName} " +
                        $"at path {reader.m_PathID}: {m_CompiledDataError}");
                }
            }
            else
