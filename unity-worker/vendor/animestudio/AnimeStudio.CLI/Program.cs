    ...
    public class Program
    {
        ...
        public static void Run(Options o)
        {
            try
            {
                ...
                // 显式操作供外部工具分两步建立并使用 CAB 映射；旧参数语义保持不变。
                if (o.MapOp.HasFlag(MapOpType.BuildCABMap))
                {
                    AssetsHelper.BuildCABMap(files, o.MapName, o.Input.FullName, game);
                }
                if (o.MapOp.HasFlag(MapOpType.UseCABMap))
                {
                    AssetsHelper.LoadCABMapInternal(o.MapName);
                    assetsManager.ResolveDependencies = true;
                }
                ...
                if (o.MapOp.Equals(MapOpType.None)
                    || o.MapOp.HasFlag(MapOpType.Load)
                    || o.MapOp.HasFlag(MapOpType.UseCABMap))
                {
                    var i = 0;
