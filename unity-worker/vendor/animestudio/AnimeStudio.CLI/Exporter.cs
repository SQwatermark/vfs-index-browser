...
namespace AnimeStudio.CLI
{
    ...
    {
        private static readonly string[] CubemapFaceNames =
        {
            "PositiveX",
            "NegativeX",
            "PositiveY",
        ...
        public static bool ExportCubemap(AssetItem item, string exportPath)
        {
            ...
            if (cubemap.m_ImageCount != CubemapFaceNames.Length ||
                faceSize <= 0 ||
            ...
            var type = Properties.Settings.Default.convertType;
            for (var faceIndex = 0; faceIndex < CubemapFaceNames.Length; faceIndex++)
            {
                var faceData = new byte[faceSize];
                Buffer.BlockCopy(imageData, faceIndex * faceSize, faceData, 0, faceSize);
                // Cubemap faces use Unity's native orientation and must not receive the Texture2D display flip.
                var image = cubemap.ConvertToImage(faceData, false);
                if (image == null)
                {
                    return false;
                }
                ...
                using (image)
                {
                    var extension = $"_{CubemapFaceNames[faceIndex]}.{type.ToString().ToLower()}";
        ...
        public static bool ExportShader(AssetItem item, string exportPath)
        {
            ...
            if (m_Shader.assetsFile.game.Type.IsArknightsEndfieldGroup())
            {
                ShaderBinaryPackageExporter.Export(m_Shader, exportFullPath + ".binary");
            }
            string str;
            try
            {
            ...
            catch (Exception ex) when (m_Shader.assetsFile.game.Type.IsArknightsEndfieldGroup())
            {
                Logger.Warning(
                    $"ShaderLab conversion is unavailable for {m_Shader.Name}; " +
                    $"the lossless binary package was preserved: {ex.Message}");
                str =
                    "// AnimeStudio: Endfield ShaderLab conversion is not implemented for this program layout.\n" +
                    "// See the adjacent .shader.binary package for lossless metadata and compiled program containers.\n" +
                    $"// Conversion error: {SanitizeShaderComment(ex.Message)}\n" +
                    $"Shader \"{EscapeShaderName(m_Shader.Name)}\" {{\n}}\n";
            }
            File.WriteAllText(exportFullPath, str);
            return true;
        }

        private static string SanitizeShaderComment(string value)
        {
        ...
        public static bool ExportIdentifiedTexture(AssetItem item, string exportPath)
        {
            if (item.Asset is not Texture2D texture)
                return false;

            var type = Properties.Settings.Default.convertType;
            var pathId = unchecked((ulong)item.m_PathID);
            if (!TryExportFile(exportPath, item, $"_p{pathId:X16}.{type.ToString().ToLower()}", out var exportFullPath))
                return false;
        ...
        public static bool ExportAnimationJSON(AssetItem item, string exportPath)
        {
            if (!(item.Asset is AnimationClip clip))
                return false;
            if (!TryExportFile(exportPath, item, ".animation.json", out var exportFullPath))
                return false;
            ...
            if (converter.Eulers.Count != 0 || converter.PPtrs.Count != 0)
            {
                throw new NotSupportedException(
                    "AnimationJSON does not yet support Euler or object-reference curves.");
            }
            uint GetPathHash(string path)
            {
                if (AnimationClipConverter.UnknownPathRegex.IsMatch(path))
                    return uint.Parse(path.Substring("path_".Length));
                ...
                .Select((binding, index) => new
                {
                    ...
                    humanoidPropertyName =
                        (BindingCustomType)binding.customType == BindingCustomType.AnimatorMuscle
                            ? binding.GetHumanoidPropertyName(
                                clip.assetsFile.game.Type.IsArknightsEndfieldGroup())
                            : null,
                })
                .ToArray();
        ...
        public static bool ExportObjectJSONFile(AssetItem item, string exportPath)
        {
            var pathId = unchecked((ulong)item.m_PathID);
            if (!TryExportFile(exportPath, item, $"_p{pathId:X16}.json", out var exportFullPath))
                return false;

            var snapshot = ObjectSnapshotExporter.Build(item);
            File.WriteAllText(exportFullPath, snapshot.ToString(Formatting.Indented));
            return true;
        }
