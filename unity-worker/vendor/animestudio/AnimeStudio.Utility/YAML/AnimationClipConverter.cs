    ...
    public class AnimationClipConverter
    {
        ...
        private void ProcessInner()
        {
            ...
            if (game.Type.IsArknightsEndfieldGroup() &&
                animationClip.m_AclCompressedBuffer?.TransformBufferData?.Length > 0)
            {
                ProcessEndfieldACL(animationClip.m_AclCompressedBuffer, bindings, tos);
        ...
        private void ProcessEndfieldACL(
            AnimClipAclCompressedBuffer acl,
            AnimationClipBindingConstant bindings,
            Dictionary<uint, string> tos)
        {
            // Root motion has no ordinary curve binding. Fail explicitly when a clip
            // declares root transforms until their dedicated mapping is implemented.
            if (acl.RootPosIndex != ushort.MaxValue ||
                acl.RootRotIndex != ushort.MaxValue ||
                acl.RootScaleIndex != ushort.MaxValue)
            {
                throw new NotSupportedException("Endfield ACL root transforms are not supported.");
            }

            var transforms = EndfieldACL.DecompressTransforms(acl.TransformBufferData);
            if (transforms.ComponentsPerTrack != 10 ||
                transforms.TrackCount != acl.OutputTrackCount)
            {
                throw new InvalidOperationException(
                    $"Unexpected Endfield transform tracks: " +
                    $"{transforms.TrackCount} x {transforms.ComponentsPerTrack}, " +
                    $"expected {acl.OutputTrackCount} x 10.");
            }

            var transformBindings = bindings.genericBindings
                .Where(binding => binding.typeID == ClassIDType.Transform)
                .GroupBy(binding => binding.attribute)
                .ToDictionary(group => group.Key, group => group.ToList());
            transformBindings.TryGetValue(1, out var orderedTransformBindings);
            if (orderedTransformBindings?.Count != transforms.TrackCount)
            {
                orderedTransformBindings = transformBindings.Values
                    .FirstOrDefault(group => group.Count == transforms.TrackCount);
            }
            if (orderedTransformBindings == null)
            {
                throw new InvalidOperationException(
                    $"No Endfield transform binding group matches " +
                    $"{transforms.TrackCount} ACL tracks.");
            }

            var bindingPathsByAttribute = transformBindings.ToDictionary(
                pair => pair.Key,
                pair => pair.Value.Select(binding => binding.path).ToHashSet());
            var slopeValues = new float[4];
            for (var frameIndex = 0; frameIndex < transforms.Times.Length; frameIndex++)
            {
                var time = transforms.Times[frameIndex];
                for (var trackIndex = 0; trackIndex < transforms.TrackCount; trackIndex++)
                {
                    var pathHash = orderedTransformBindings[trackIndex].path;
                    var path = GetCurvePath(tos, pathHash);
                    var valueOffset =
                        (frameIndex * transforms.TrackCount + trackIndex) *
                        transforms.ComponentsPerTrack;

                    AddEndfieldTransformCurve(
                        bindingPathsByAttribute,
                        pathHash,
                        time,
                        transforms.Values,
                        valueOffset,
            ...
            var floatBindings = bindings.genericBindings
                .Where(binding => binding.typeID != ClassIDType.Transform)
                .ToList();
            if (acl.FloatBufferData.Length == 0)
            {
                if (floatBindings.Count != 0)
                    throw new InvalidOperationException("Endfield float bindings have no ACL buffer.");
                return;
            }

            var floats = EndfieldACL.DecompressFloats(acl.FloatBufferData);
            if (floats.ComponentsPerTrack != 1 ||
                floats.TrackCount != acl.FloatCurveCount ||
                floats.TrackCount != floatBindings.Count ||
                !floats.Times.SequenceEqual(transforms.Times))
            {
                throw new InvalidOperationException(
                    $"Unexpected Endfield float tracks: {floats.TrackCount} bindings, " +
                    $"{floatBindings.Count} declared bindings, " +
                    $"{acl.FloatCurveCount} declared curves.");
            }

            for (var frameIndex = 0; frameIndex < floats.Times.Length; frameIndex++)
            {
                var time = floats.Times[frameIndex];
                var frameOffset = frameIndex * floats.TrackCount;
                for (var trackIndex = 0; trackIndex < floats.TrackCount; trackIndex++)
                {
                    var binding = floatBindings[trackIndex];
                    var path = GetCurvePath(tos, binding.path);
                    var value = floats.Values[frameOffset + trackIndex];
        ...
        private void AddEndfieldTransformCurve(
            Dictionary<uint, HashSet<uint>> bindingPathsByAttribute,
            uint pathHash,
            float time,
            float[] values,
            int valueOffset,
            ...
            string path)
        {
            foreach (var pair in bindingPathsByAttribute)
            {
                if (!pair.Value.Contains(pathHash))
                    continue;

                var componentOffset = pair.Key switch
                {
                    1 => 0,
                    2 => 3,
                    3 => 7,
                    _ => throw new InvalidOperationException(
                        $"Unsupported Endfield transform attribute {pair.Key}."),
                };
                AddTransformCurve(
                    time,
                    pair.Key,
...
#warning TODO:
        ...
        private void AddAnimatorMuscleCurve(GenericBinding binding, float time, float value)
        {
            FloatCurve curve = new FloatCurve(
                string.Empty,
                binding.GetHumanoidPropertyName(game.Type.IsArknightsEndfieldGroup()),
                ClassIDType.Animator,
                new PPtr<MonoScript>(0, 0, null));
            AddFloatKeyframe(curve, time, value);
        }
