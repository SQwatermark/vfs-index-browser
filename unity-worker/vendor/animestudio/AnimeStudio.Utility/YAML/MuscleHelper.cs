...
namespace AnimeStudio
{
    ...
    {
        private static readonly string[] EndfieldExtraMuscleNames =
        {
        ...
        public static string ToEndfieldAttributeString(int attribute)
        {
            const int muscleAttributeOffset = 42;
            if (attribute < muscleAttributeOffset)
                return ((HumanoidMuscleType)attribute).ToAttributeString();

            var slot = attribute - muscleAttributeOffset;
            if (slot < 0 || slot >= 101)
                throw new ArgumentException(attribute.ToString());

            var extraIndex = slot switch
            {
                28 => 0,
                30 => 1,
                31 => 2,
            ...
            if (extraIndex >= 0)
                return EndfieldExtraMuscleNames[extraIndex];

            if (slot <= 27)
                return ((MuscleType)slot).ToAttributeString();
            if (slot == 29)
                return MuscleType.LeftToesUpDown.ToAttributeString();
            if (slot >= 32 && slot <= 38)
                return ((MuscleType)(slot - 3)).ToAttributeString();
            if (slot == 40)
                return MuscleType.RightToesUpDown.ToAttributeString();
            if (slot >= 43 && slot <= 60)
                return ((MuscleType)(slot - 6)).ToAttributeString();

            var finger = (HumanoidMuscleType)((int)HumanoidMuscleType.Fingers + slot - 61);
            return finger.ToAttributeString();
        }
