...
def build_input(
    ...
    *,
    ...
    if len(skeleton["m_Node"]) != len(skeleton["m_ID"]) or len(skeleton["m_ID"]) != len(skeleton_pose):
    ...
    for index, (node, node_id, transform) in enumerate(
        ...
        if parent_index >= index:
            raise ValueError(f"Avatar Humanoid skeleton node {index} has an invalid parent")
        avatar_pose.append(
            {
                "name": path.rsplit("/", 1)[-1],
                "parentName": avatar_pose[parent_index]["name"] if parent_index >= 0 else "",
