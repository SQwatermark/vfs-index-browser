...
def parse_avatar_mesh(text: str) -> dict:
    ...
    missing = [key for key in ("name", "mainPrefabPath", "mainPrefabPathHash") if result[key] is None]
    if missing:
...
def attach_resolved_paths(document: dict, index: StringPathHashIndex) -> None:
    ...
    document["mainPrefabResolvedPaths"] = list(resolved[document["mainPrefabPathHash"]])
    for slot in document["slots"]:
        for meshes in slot["lods"].values():
            for mesh in meshes:
